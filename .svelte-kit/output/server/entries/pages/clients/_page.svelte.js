...
function _page($$payload, $$props) {
  ...
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    ...
    $$payload.out += `<tr class="border-b"><td class="p-2">${escape_html(row.name)}</td><td class="p-2">${escape_html(row.pivot)}</td><td class="p-2">${escape_html(row.totalBotte)}</td><td class="p-2">${escape_html(row.datefrom)}</td><td class="p-2">${escape_html(row.dateto)}</td><td class="p-2">`;
