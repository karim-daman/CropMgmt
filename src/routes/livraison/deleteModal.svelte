<script lang="ts">
	...
	function deleteRow() {
		let tempObj = $livraisons.find((row) => row.ID == ID);
		$livraisons = $livraisons.filter((row) => row.ID !== ID);

		toast.success('Removed a livraison ID: ' + ID, {
			position: 'top-right'
		});
		hidden = true;

		let newHistoryItem: Action = {
			name: 'Deleted a livraison with ID: ' + ID,
			date: new Date(),
			status:
		...
		<div class="relative rounded-lg bg-white shadow-sm dark:bg-gray-700">
			...
			<div class="p-4 text-center md:p-5">
				...
				<h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
					Are you sure you want to delete livraison ID: {ID}?
				</h3>
				<button
					onclick={() => {
						deleteRow();
